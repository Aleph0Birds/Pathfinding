...
void AStar::extractNeighbors(const Node *node) {
    ...
    // Add neighbors
    if (x > 0) {
        addNode(x - 1, y, calculateScore(x-1,y), x, y);
    }
    if (x < grid->tilesX - 1) {
        addNode(x + 1, y, calculateScore(x+1,y), x, y);
    }
    if (y > 0) {
        addNode(x, y - 1, calculateScore(x,y-1), x, y);
    }
    if (y < grid->tilesY - 1) {
        addNode(x, y + 1, calculateScore(x,y+1), x, y);
    }
...
int AStar::calculateScore(const int x, const int y) const {
    const int scoreBegin = std::abs(x - grid->beginX) +
        std::abs(y - grid->beginY);
    const int scoreEnd = std::abs(x - grid->endX) +
        std::abs(y - grid->endY);
    return scoreEnd + scoreBegin + grid->getTile(x, y).node->cost;
}

void AStar::addNode(int x, int y, int curScore, int parentX, int parentY) {
    ...
    node->tileY = y;
    node->score = curScore;
    node->cost = grid->getTile(x, y).node->cost+1;
